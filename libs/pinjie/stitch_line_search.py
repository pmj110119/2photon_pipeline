...
def search_line(pre_img1x, pre_img2x, pre_img1y, pre_img2y):
    ...
    pre_img1x_t=cv2.GaussianBlur(pre_img1x,(3,3),1.0)
    pre_img2x_t=cv2.GaussianBlur(pre_img2x,(3,3),1.0)
    pre_img1y_t=cv2.GaussianBlur(pre_img1y,(3,3),1.0)
    pre_img2y_t=cv2.GaussianBlur(pre_img2y,(3,3),1.0)
    # pre_img1x_t=pre_img1x
    # pre_img2x_t=pre_img2x
    # pre_img1y_t=pre_img1y
    # pre_img2y_t=pre_img2y
    y, x = pre_img1x.shape[:]
    model = [1, 2, 3, 2, 1]
    line = []
    for i in range(x - top_start*2):
        ...
        dot1x = np.dot(pre_img1x_t[0:1, i:i + 5], model)
        dot2x = np.dot(pre_img2x_t[0:1, i:i + 5], model)
        dot1y = np.dot(pre_img1y_t[0:1, i:i + 5], model)
        dot2y = np.dot(pre_img2y_t[0:1, i:i + 5], model)

        correlation12 = np.abs((dot1x - dot2x ) / (dot1x + dot2x + 0.01)) + \
                        np.abs((dot1y - dot2y ) / (dot1y + dot2y + 0.01))
    ...
    for i in range(y):
        ...
        for j in range(len(model)):
            ...
            dot1x = np.dot(pre_img1x_t[i:i + 1, j:j + len(model)], model)
            dot2x = np.dot(pre_img2x_t[i:i + 1, j:j + len(model)], model)
            dot1y = np.dot(pre_img1y_t[i:i + 1, j:j + len(model)], model)
            dot2y = np.dot(pre_img2y_t[i:i + 1, j:j + len(model)], model)
...
def mix_img(img1, img2, line):
    y, x = img1.shape[0:2]
    for i in range(y):
        img1[i, line[i]:x] = img2[i, line[i]:x]
